...
`scripts/` holds prek hook implementations — not standalone scripts. Never invoke them directly
(with `python`, `uv run python`, or any other method); use `uv run prek run <hook-id>` instead.
